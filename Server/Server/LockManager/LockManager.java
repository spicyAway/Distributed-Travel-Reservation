				...
				else if (dataLockObject.getLockType() == TransactionLockObject.LockType.LOCK_WRITE)
				{
					...
					if (l_dataLockObject.getLockType() == TransactionLockObject.LockType.LOCK_WRITE){
						...
						for(int j=0; j<size; j++){
							DataLockObject check_dataLockObject = (DataLockObject)vect.elementAt(i);
							if(dataLockObject.getXId() != check_dataLockObject.getXId()){
								Trace.info("LM::lockConflict(" + dataLockObject.getXId() + ", " + dataLockObject.getDataName() + ") Want WRITE, Already has READ, someone has WRITE/READ");
								return true;
							}
						}
					}
					this.lockTable.remove(dataLockObject);
					TransactionLockObject remove_query = new TransactionLockObject(dataLockObject.getXId(),dataLockObject.getDataName(), dataLockObject.getLockType());
					this.lockTable.remove(remove_query);
					bitset.set(0, true);
					//TODO: Lock conversion
				}
			}
